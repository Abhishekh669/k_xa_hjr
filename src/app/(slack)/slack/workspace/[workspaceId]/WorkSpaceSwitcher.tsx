      ...
      <DropdownMenuTrigger asChild>
        <Button className="size-9  justify-center relative overflow-hidden rounded-[5px] bg-[#ABABAD] hover:bg-[#ABABAD]/80 text-slate-800 font-semibold text-xl">
          {workspaceLoading ? (
            ...
            <span>{workspace?.workspace?.workspaceId?.name.charAt(0).toUpperCase()}</span>
          )}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent
        side="bottom"
        align="start"
        className="w-64 bg-white flex flex-col rounded-[5px] hover:bg-white hover:text-black"
      >
        <DropdownMenuItem
          className=" window-switch  hover:text-black text-black cursor-pointer flex-col justify-start items-start capitalize   z-10 "
